...
setmem v blocksize = concat [ buftxt i | i <- [1..blocksize -1]]
    where buftxt n = "buf" ++ v ++ show n ++ "[" ++ show (n -1) ++ "] = mem" ++  v ++ "[" ++ show n ++"];\n"

setaccumx blocksize =  "accum[0][0][0] = memx[0];\n" ++ concat [ buftxt i | i <- [1..blocksize -1]]
    where buftxt n = "accum[0][" ++ show n ++ "][0] = bufx" ++ show n ++ "[0];\n"

setaccumy blocksize =  "accum[0][0][1] = memy[0];\n" ++ concat [ buftxt i | i <- [1..blocksize -1]]
                 ...
                 "for( int baselineBlock = 0 ; baselineBlock < NR_BLOCKS; baselineBlock++) {\n" ++
                  indent (
                    ...
                    "for (int time = 0 ; time < NR_SAMPLES_PER_CHANNEL + BLOCK_SIZE; time++){\n" ++ 
                      ...
                      getMem ++
                      shiftx ++ shifty ++ 
                      setaccumx blocksize ++ setaccumy blocksize ++
                      shiftbuf "x" blocksize ++ shiftbuf "y" blocksize ++
                      setmem "x" blocksize ++ setmem "y" blocksize ++
                      addAll) ++
                    "}\n" ++
                    writemem ++ addIndices
                   ) ++ "}\n"
